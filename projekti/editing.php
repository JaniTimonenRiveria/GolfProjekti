...
if (isset($date) && !empty($date)) {
    ...
    $stmt->bind_param("sssisssii", $tee, $gamemode, $date, $swings, $hole, $username, $team, $points, $par);
} 

else {
    $stmt = $connect->prepare("INSERT INTO live (tee, gamemode, swings, hole, username, team, points, par) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssisssii", $tee, $gamemode, $swings, $hole, $username, $team, $points, $par);
}

$stmt->execute();
    header("Location: /projekti/livepage.php");
